...
// 1. Get list of settings (with filter support)
export const useCurrencyExchangeSettings = (
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.admin.currencyExchangeSettings.list(
        queryParams
      );
...
// 2. Get a single setting by id
export const useCurrencyExchangeSetting = (id: string, queryParams = {}) => {
  const { data, isLoading, error } = useQuery({
    queryKey: ["currency-exchange-setting", id, queryParams],
    queryFn: async () => {
      const response = await sdk.admin.currencyExchangeSettings.retrieve(
        id,
        queryParams
      );
...
// 3. Enable a currency (create new)
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["currency-exchange-settings", "list"],
      });
    },
  });
};

// 4. Update exchange rate or mode
export const useUpdateCurrencyExchangeSetting = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({
        queryKey: ["currency-exchange-setting", id],
      });
      queryClient.invalidateQueries({
        queryKey: ["currency-exchange-settings", "list"],
      });
...
// 5. (Optional) Disable currency exchange (if API supports it)
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await fetch(
        "/admin/currency-exchange-settings/update-currency-rates",
        { method: "POST" }
      );
